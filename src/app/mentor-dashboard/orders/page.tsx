...
export default function MentorOrdersPage() {
  ...
  const tSessionStatusLabel = lang === 'en' ? sessionStatusLabelEn : sessionStatusLabel;
  const tStatusLabel = lang === 'en' ? statusLabelEn : statusLabel;
  const tPayMethodLabel = lang === 'en' ? payMethodLabelEn : payMethodLabel;
  ...
  const loadFinancial = useCallback(async () => {
    ...
    try {
      const token = await authUser.getIdToken();
      const r = await fetch('/api/mentor/financial', { headers: { authorization: `Bearer ${token}` } });
    ...
    try {
      ...
      if (status === 'confirmed') {
        if (data.isGuest) {
          ...
          toast({ title: bi('تم تأكيد الطلب', 'Order confirmed'), description: bi('تم إرسال إشعار للمشترك برابط الدورة.', 'A notification with the course link was sent to the buyer.') });
        }
      } else {
  ...
  const exportStatusLabel = (o: FinancialOrder) =>
    o.type === 'session'
      ? (tSessionStatusLabel[o.status]?.label ?? o.status)
          ...
          {!loading && pending.length > 0 && (
            <div>
              <h2 className="text-base font-semibold mb-3 flex items-center gap-2">
                ...
                {bi("تحتاج إلى تأكيد", "Need confirmation")} ({pending.length})
              </h2>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {bi(`عمولة EmpowerHub على مبيعات الدورات: ${commissionRate}% — بالإضافة لمستحقات جلسات الإرشاد حسب سعر الساعة ونسبة المنظمة`, `EmpowerHub's commission on course sales: ${commissionRate}% — plus mentoring session earnings based on hourly rate and the organization's cut`)}
              </p>
            </div>
            ...
            <CardContent className="p-0">
              {financialLoading ? (
                ...
                <div className="py-12 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">{bi("لا توجد طلبات أو جلسات مالية بعد", "No orders or financial sessions yet")}</p>
                </div>
              ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {financialOrders.map(order => {
                        ...
                        const st = isSession
                          ? (tSessionStatusLabel[order.status] ?? { label: order.status, className: 'bg-muted text-muted-foreground' })
                          ...
                          <TableRow key={order.id}>
                            ...
                            <TableCell className="text-sm font-medium">
                              ...
                              {isSession && order.hours != null && (
                                <span className="text-xs text-muted-foreground mr-1">({order.hours} {bi("ساعة", "hrs")})</span>
                              )}
                            </TableCell>
...
function OrderCard({
  ...
  const bi = (ar: string, en: string) => (lang === 'en' ? en : ar);
  const locale = lang === 'en' ? 'en-US' : 'ar-EG';
  const st = (lang === 'en' ? statusLabelEn : statusLabel)[order.status] ?? { label: order.status, className: 'bg-muted text-muted-foreground' };
  const date = order.createdAt ? new Date(order.createdAt).toLocaleDateString(locale) : '';
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3">
          <div className="flex-1 min-w-0 space-y-1.5">
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
              ...
              <span>{(lang === 'en' ? payMethodLabelEn : payMethodLabel)[order.paymentMethod] || order.paymentMethod}</span>
              {date && <span>{date}</span>}
            </div>
            {order.amount > 0 && (
          ...
          {order.status === 'pending' && onConfirm && onReject && (
            <div className="flex gap-2 shrink-0">
              ...
              >
                ...
                {updating ? bi('جاري...', 'Working...') : bi('تأكيد', 'Confirm')}
              </Button>
              <Button
                size="sm"
                variant="outline"
