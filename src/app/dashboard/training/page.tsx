...
type Course = { id: string; title: string; description?: string; category?: string; status: string; progress: number };

const EmptyState = ({ title, description }: { title: string; description: string }) => (
  <div className="col-span-full flex flex-col items-center justify-center text-center p-12 bg-muted/40 rounded-xl border border-dashed">
    <div className="p-4 bg-primary/10 rounded-full mb-4"><BookHeart className="h-10 w-10 text-primary" /></div>
    <h3 className="text-lg font-semibold">{title}</h3>
    <p className="text-muted-foreground mt-2 max-w-sm text-sm leading-relaxed">{description}</p>
  </div>
);

const CourseCard = ({ course, completed = false }: { course: Course; completed?: boolean }) => (
  <Card className={`card-hover flex flex-col border-0 shadow-sm ${completed ? "bg-muted/30" : "bg-card"}`}>
    <CardHeader>
      ...
      {course.description && <CardDescription className="text-sm leading-relaxed line-clamp-2 pr-12">{course.description}</CardDescription>}
    </CardHeader>
    <CardContent className="flex-grow">
      <div className="space-y-2">
        ...
        {!completed && course.progress > 0 && (
          <p className="text-xs text-primary flex items-center gap-1"><Clock className="h-3 w-3" />جارٍ — تبقى {100 - course.progress}% للإكمال</p>
        )}
      </div>
    </CardContent>
    <CardFooter>
      <Button className="w-full" variant={completed ? "secondary" : "default"} asChild>
        <Link href={`/dashboard/training/${course.id}`}>
          {completed ? <><BookMarked className="ml-2 h-4 w-4" />مراجعة الدورة</> : course.progress > 0 ? <><PlayCircle className="ml-2 h-4 w-4" />متابعة الدورة</> : <><PlayCircle className="ml-2 h-4 w-4" />ابدأ الدورة</>}
        </Link>
      </Button>
    </CardFooter>
  </Card>
...
const SkeletonCard = () => (
  <Card className="border-0 shadow-sm">
    <CardContent className="p-6 space-y-3">
      <Skeleton className="h-5 w-3/4" /><Skeleton className="h-4 w-full" /><Skeleton className="h-2 w-full mt-2" /><Skeleton className="h-10 w-full mt-2" />
    </CardContent>
  </Card>
);
...
export default function TrainingPage() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/beneficiary/courses', { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{loading ? "جاري التحميل..." : `${totalCompleted} من ${courses.length} دورات مكتملة`}</p>
        </div>
        ...
        <Card className="border-0 shadow-sm bg-gradient-to-l from-primary/5 to-accent/5">
          <CardContent className="pt-5 pb-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-bold text-primary">{Math.round((totalCompleted / courses.length) * 100)}%</span>
            </div>
            <Progress value={Math.round((totalCompleted / courses.length) * 100)} className="h-3" />
      ...
      {(loading || inProgress.length > 0) && (
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {loading ? [...Array(2)].map((_, i) => <SkeletonCard key={i} />) : inProgress.map(c => <CourseCard key={c.id} course={c} />)}
          </div>
        </section>
      )}
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {loading ? [...Array(3)].map((_, i) => <SkeletonCard key={i} />) : notStarted.length > 0 ? notStarted.map(c => <CourseCard key={c.id} course={c} />) : <EmptyState title="لا توجد دورات متاحة" description="لم يتم تعيين أي دورات لك بعد. تواصل مع مدير منظمتك." />}
        </div>
      </section>
      ...
      {(loading || completed.length > 0) && (
        <section>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            ...
            {completed.length > 0 && <Badge className="bg-primary/10 text-primary border-primary/20">{completed.length}</Badge>}
          </h2>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {loading ? <SkeletonCard /> : completed.map(c => <CourseCard key={c.id} course={c} completed />)}
          </div>
        </section>
      )}
    </div>
