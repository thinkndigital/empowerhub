...
export default function CoachSessionsPage() {
  ...
  const { upcomingSessions, pastSessions } = useMemo(() => {
    ...
    sessions.forEach(s => {
      const sw = { ...s, beneficiaryName: beneficiaryMap.get(s.attendees?.[0]) || bi('متدرب', 'Beneficiary') };
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (editingSession) {
        ...
        if (!res.ok) throw new Error((await res.json()).error);
        toast({ title: bi("تمت الجدولة!", "Scheduled!"), description: bi(`تم جدولة جلستك "${values.title}".`, `Your session "${values.title}" has been scheduled.`) });
      }
      setIsDialogOpen(false);
      setEditingSession(null);
      form.reset();
      fetchData();
    } catch (e: any) { toast({ variant: 'destructive', title: bi('خطأ', 'Error'), description: e.message }); }
  }
  ...
  async function handleUpdateSessionStatus(sessionId: string, status: 'completed' | 'cancelled') {
    ...
    try {
      ...
      fetchData();
    } catch { toast({ variant: 'destructive', title: bi('خطأ', 'Error'), description: bi('فشل تحديث حالة الجلسة.', 'Failed to update session status.') }); }
  }
      ...
      <div className="space-y-6" dir={dir}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{bi('إدارة وجدولة جلسات التدريب مع المتدربين.', 'Manage and schedule coaching sessions with beneficiaries.')}</p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) { setEditingSession(null); form.reset(); } }}>
                <DialogTrigger asChild><Button><PlusCircle className="ml-2 h-4 w-4" />{bi('جدولة جلسة جديدة', 'Schedule New Session')}</Button></DialogTrigger>
                <DialogContent dir={dir} className="sm:max-w-[90vw] md:max-w-[600px]" onPointerDownOutside={(e) => { if (e.target instanceof Element && e.target.closest('.rdp')) e.preventDefault(); }}>
                  <DialogHeader><DialogTitle>{editingSession ? bi('تعديل الجلسة', 'Edit Session') : bi('جدولة جلسة جديدة', 'Schedule New Session')}</DialogTitle><DialogDescription>{editingSession ? bi('عدّل تفاصيل الجلسة ثم احفظ التغييرات.', 'Edit the session details then save your changes.') : bi('املأ التفاصيل لجدولة جلسة تدريبية جديدة.', 'Fill in the details to schedule a new coaching session.')}</DialogDescription></DialogHeader>
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                      <FormField control={form.control} name="beneficiaryId" render={({ field }) => (
                        <FormItem><FormLabel>{bi('المتدرب', 'Beneficiary')}</FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder={bi("اختر متدربًا", "Select a beneficiary")} /></SelectTrigger></FormControl>
                            <SelectContent>
                              {loading ? <SelectItem value="l" disabled>{bi('جاري التحميل...', 'Loading...')}</SelectItem> :
                                beneficiaries.map(b => <SelectItem key={b.id} value={b.id}>{b.name}</SelectItem>)}
                            </SelectContent>
                          </Select><FormMessage /></FormItem>
                      )} />
                      <FormField control={form.control} name="title" render={({ field }) => (
                        <FormItem><FormLabel>{bi('عنوان الجلسة', 'Session Title')}</FormLabel><FormControl><Input placeholder={bi("مثال: مراجعة مهارات القيادة", "e.g. Leadership Skills Review")} {...field} /></FormControl><FormMessage /></FormItem>
                      )} />
                      <div className="grid grid-cols-2 gap-4">
                            ...
                            <Popover><PopoverTrigger asChild>
                              <FormControl>
                                <Button variant="outline" className={cn("pr-3 text-right font-normal", !field.value && "text-muted-foreground")}>
                                  {field.value ? format(field.value, "PPP", { locale: dfLocale }) : <span>{bi('اختر تاريخًا', 'Pick a date')}</span>}
                        ...
                        <FormField control={form.control} name="time" render={({ field }) => (
                          <FormItem><FormLabel>{bi('وقت الجلسة', 'Session Time')}</FormLabel><FormControl><Input type="time" {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                      </div>
                      <FormField control={form.control} name="duration" render={({ field }) => (
                        <FormItem><FormLabel>{bi('المدة (بالدقائق)', 'Duration (minutes)')}</FormLabel><FormControl><Input type="number" placeholder="60" {...field} /></FormControl><FormMessage /></FormItem>
                      )} />
                      <FormField control={form.control} name="meetLink" render={({ field }) => (
                        <FormItem><FormLabel>{bi('رابط Google Meet (اختياري)', 'Google Meet Link (optional)')}</FormLabel>
                          ...
                          {!googleToken ? (
                            <Button type="button" variant="secondary" size="sm" className="mt-2 w-full" onClick={handleLinkGoogle} disabled={googleLinking}>
                              {googleLinking ? bi('جارٍ الربط...', 'Linking...') : bi('ربط حساب Google لإنشاء رابط Meet تلقائياً', 'Link Google account to auto-generate Meet links')}
                            </Button>
                          ) : (
                      ...
                      <DialogFooter>
                        ...
                        <Button type="submit">{editingSession ? bi('حفظ التعديلات', 'Save Changes') : bi('جدولة', 'Schedule')}</Button>
                      </DialogFooter>
                    </form>
                  </Form>
                </DialogContent>
              ...
              <div key={session.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 rounded-lg bg-muted/50">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground mt-1">
                    ...
                    <span className="flex items-center gap-1.5"><CalendarIcon className="h-4 w-4" />{format(safeDate(session.date), "d MMMM yyyy", { locale: dfLocale })}</span>
                    <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" />{format(safeDate(session.date), "p", { locale: dfLocale })}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <Button size="sm" asChild disabled={!session.meetLink}><a href={session.meetLink} target="_blank" rel="noopener noreferrer"><Video className="ml-2 h-4 w-4" />{bi('انضم للجلسة', 'Join Session')}</a></Button>
                  <DropdownMenu>
                ...
                <div key={session.id} className="p-4 rounded-lg border bg-card flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1.5 flex-wrap">
                      ...
                      <span className="flex items-center gap-1.5"><CalendarIcon className="h-3.5 w-3.5" />{format(sessionDate, "d MMMM yyyy", { locale: dfLocale })}</span>
                      <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" />{format(sessionDate, "p", { locale: dfLocale })}</span>
                      {session.duration && (
                        <span className="flex items-center gap-1.5 text-xs">{session.duration} {bi('دقيقة', 'min')}</span>
                      )}
                    </div>
                    {isCompleted && (
                      <div className="mt-2">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-green-600">{bi('مكتملة', 'Completed')}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  {isCompleted && (
                    <Button variant="outline" size="sm" className="shrink-0" onClick={() => setEvaluationTarget({ sessionId: session.id, evaluatedId: session.attendees[0], evaluatedName: session.beneficiaryName || bi('المتدرب', 'Beneficiary') })}>
                      <Star className="ml-2 h-4 w-4" />{bi('تقييم المتدرب', 'Evaluate Beneficiary')}
                    </Button>
                  )}
                </div>
              );
